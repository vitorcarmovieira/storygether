...
class SMSegment: UIView {
    ...
    private func resetContentFrame() {
        ...
        var imageViewFrame = CGRectMake(0.0, self.verticalMargin, 0.0, self.frame.size.height / 2.0)
        
        
        if self.onSelectionImage != nil || self.offSelectionImage != nil {
            // Set imageView as a square
            imageViewFrame.size.width = self.frame.size.height / 2.0
            distanceBetween = 5.0
        }
        ...
        self.label.frame = CGRectMake(imageViewFrame.origin.x, self.imageView.frame.height + self.imageView.frame.origin.y, self.labelWidth, self.frame.height/3)
